...
class Lab10ApplicationTests {
    ...
    void testSave() {
        ...
        Product newProduct = new Product("99", "Test Product",
                "Books", "TestBrand", 5, 100.0, "NONE");

        StepVerifier.create(repo.save(newProduct))
                .expectNextMatches(p -> p.getId().equals("99"))
                .verifyComplete();

        // ตรวจว่าบันทึกลง store จริง
        StepVerifier.create(repo.findById("99"))
                .expectNextCount(1)
                .verifyComplete();
    }
